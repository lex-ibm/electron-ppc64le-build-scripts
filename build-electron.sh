...
patch -p1 -R < "${patches_dir}"/fedora/chromium-141-revert-remove-darkmode-image-policy.patch # Patch94 P94
...
patch -p1 -R < "${patches_dir}"/fedora/chromium-131-revert-decommit-pooled-pages-by-default.patch # Patch300 P300
...
###patch -p1 < "${patches_dir}"/fedora/chromium-136-rust-skrifa-build-error.patch # Patch314 P314 # Doesn't apply
...
gn gen "${electron_out}" --args="import(\"//electron/build/args/${RELEASE_TYPE}.gn\") ${ELECTRON_GN_DEFINES}"

# Build Electron
NINJA_SUMMARIZE_BUILD=1 ninja -j "$(nproc)" -C "${electron_out}" "electron:${RELEASE_TYPE}_build"
